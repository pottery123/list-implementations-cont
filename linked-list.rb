...
class LinkedList
  ...
  def delete(val)
    return nil if @size == 0#if the size is zero get out
    if @head.value == val
      ...
      if current != nil
        previous.next_node = current.next_node #skips over the node
        @size -= 1
      end
    end
  end
  ...
  def include?(key)
    current = @head #DON'T LOOSE THE HEAD
    while current != nil
  ...
  def sort
    current = @next_node
    return current  if node.next.nil?
